...
void* ppn_to_ptr(int ppn){
    return reinterpret_cast<void*>(reinterpret_cast<const uintptr_t>(vm_physmem) + (ppn));
}
...
int vm_fault(const void* addr, bool write_flag){
    ...
    if(vpn_d.state == 4){
        if(write_flag){
            ...
            auto &cur = vpn_data_tables[current_pid][vpn];
            cout << "got here" << endl;
            cout << "filename " << " cur block " << cur.block << " ppn " << ppn << endl;
            cout << reinterpret_cast<void*>(static_cast<char *>(vm_physmem)[ppn]) << endl;
            file_read(cur.filename, cur.block, reinterpret_cast<void*>(static_cast<char *>(vm_physmem)[ppn]));
        }
    }
    if(vpn_d.state == 3){
        vpn_d.pte->read_enable = 1;
