...
int evict(){
    while(!clock_q.empty()){
        ...
        if(ppn_clock[clock_q.front()].referenced == false) break;
            ...
            if(vpn_d->state == 5){
                vpn_d->state = 6;
                assert(ppn_clock[cur_ppn].dirty == false);
            }
            if(vpn_d->state == 2){
                vpn_d->state = 3;
                assert(ppn_clock[cur_ppn].dirty == true);
            }
            vpn_d->pte->read_enable = 0;
            vpn_d->pte->write_enable = 0;

            if(ppn_clock[cur_ppn].is_file_backed == true){ // swap backed
                copy_to_refs(vpn_d);
            }
        }
    }
...
void* vm_map(const char* filename, unsigned int block){
    if(filename == nullptr){ // SWAP BACKED
        ...
        if(filenamepair.second == false) return nullptr;
