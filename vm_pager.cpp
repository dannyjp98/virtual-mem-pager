...
void print_mem(int index){
    cout << endl;
    // auto cur_block = static_cast<char*>(vm_physmem) + (index * VM_PAGESIZE)
    for(int i = 0; i<VM_PAGESIZE; i++){
        cout << static_cast<char*>(vm_physmem) + ((index * VM_PAGESIZE) + i) << endl;
    }
    // for(int i = 100; i<VM_PAGESIZE; i = i + 100){
    //     cout << static_cast<char*>(vm_physmem)[index];
    // }
    cout << endl;
}
